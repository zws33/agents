...
- **Bound the investigation.** Explore only as far as needed to form a reliable system model: entry points -> dependency graph -> data flow -> key abstractions -> invariants -> test coverage. Stop when the model is good enough to answer the question asked, not when the codebase is exhausted. Use `grep_search`, `glob`, and `read_file` to explore.
